...
def kpis(dias: int = 30, usuario=Depends(solo_ger), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "penalizaciones_mes": db.query(func.count(m.Penalizacion.id)).filter(
            m.Penalizacion.fecha_generacion >= desde.date()).scalar() or 0,
...
def atender_alerta(alerta_id: int, nota: str = "", usuario=Depends(solo_ger),
    ...
    a.atendida = True
    a.fecha_atencion = datetime.utcnow()
...
def ranking_penalizaciones(usuario=Depends(solo_ger), db: Session = Depends(get_db)):
    filas = (db.query(m.Penalizacion.chofer_id, func.count(m.Penalizacion.id))
             .group_by(m.Penalizacion.chofer_id).all())
    out = [{"chofer_id": cid, "chofer": svc.nombre_chofer(db, cid), "total": n}
           for cid, n in filas]
