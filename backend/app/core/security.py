...
def registrar_bitacora(db: Session, usuario_id, accion: str, entidad_tipo: str = None,
    ...
    `datos_antes` es opcional pero importa en los cambios que hay que poder
    reconstruir: mover una cita, cambiar un poseedor, corregir un monto. Sin el
    valor anterior el asiento dice QUE cambio pero no DESDE QUE, y asi no sirve
    para auditar. La columna existia desde el principio y nadie la llenaba.
    """
    ...
    db.add(BitacoraAuditoria(
        ...
        entidad_id=entidad_id, datos_antes=datos_antes, datos_despues=datos_despues,
    ))
