...
def mis_citas(usuario=Depends(solo_chofer), db: Session = Depends(get_db)):
    """Las citas de taller de la unidad que traigo hoy.

    Van contra la unidad de la que soy POSEEDOR, no de la que soy titular: si
    me prestaron la unidad, la cita la tengo que atender yo.
    """
    u = _mi_unidad(db, usuario.id)
    if not u:
        return []
    citas = (db.query(m.CitaTaller)
             .filter(m.CitaTaller.unidad_id == u.id,
                     m.CitaTaller.estado.in_(("propuesta", "confirmada", "reprogramada")))
             .order_by(m.CitaTaller.fecha_cita).all())
    return [agenda.cita_out(db, c) for c in citas]


@router.post("/citas/{cita_id}/confirmar", response_model=CitaOut)
def confirmar_cita(cita_id: int, usuario=Depends(solo_chofer),
                   db: Session = Depends(get_db)):
    """CU-CHO-07: el chofer confirma que se va a presentar.

    Es la mitad que le toca del mecanismo nuevo. Antes el chofer SOLICITABA el
    ingreso preventivo, y como el taller le cuesta comisiones podia "olvidarlo".
    Ahora Victor agenda y el chofer solo confirma, con lo que el conflicto de
    interes sale del camino critico.

    Solo puede confirmar citas de la unidad que trae hoy: la responsabilidad
    sigue a quien conduce, y confirmar por otro seria firmar por otro.
    """
    c = db.query(m.CitaTaller).filter(m.CitaTaller.id == cita_id).first()
    if not c:
        raise HTTPException(404, "La cita no existe")
    u = _mi_unidad(db, usuario.id)
    if not u or c.unidad_id != u.id:
        raise HTTPException(403, "Esa cita no es de la unidad que traes")
    if c.estado in ("cumplida", "no_asistio", "cancelada"):
        raise HTTPException(409, f"La cita ya esta {c.estado}")

    c.fecha_confirmacion_chofer = ahora_utc()
    registrar_bitacora(db, usuario.id, "cita_confirmada_chofer", "cita_taller", c.id)
    db.commit()
    db.refresh(c)
...
def iniciar_jornada(checklist_ok: bool = True, notas: str = "",
    ...
    if u.estado == "disponible":
        u.estado = "en_ruta"
    db.flush()
    svc.sincronizar_poseedor(db, u)   # la jornada manda sobre prestamo y titularidad
...
def cerrar_jornada(km_fin: int | None = None, usuario=Depends(solo_chofer),
    ...
    db.flush()
    svc.sincronizar_poseedor(db, j.unidad)   # al cerrar, manda el prestamo o el titular
    db.commit()
...
def crear_solicitud(datos: SolicitudIn, usuario=Depends(solo_chofer),
    ...
    # Una unidad que YA esta en el taller no puede pedir entrar. Antes se
    # dejaba levantar la solicitud y el bloqueo aparecia hasta que el
    # administrador intentaba aceptarla: el chofer se quedaba esperando una
    # respuesta a algo que nunca se iba a poder aceptar, y la bandeja de Pedro
    # se llenaba de solicitudes muertas. El "no" tiene que llegar aqui.
    dentro = svc.orden_abierta_de_unidad(db, unidad.id)
    if dentro:
        taller_nombre = dentro.taller.nombre if dentro.taller else "el taller"
        raise HTTPException(409, {
            "mensaje": f"Tu unidad {unidad.num_economico} ya esta en el taller de "
                       f"{taller_nombre} con la orden {dentro.folio}. No puedes pedir "
                       "otro ingreso hasta que salga.",
            "orden_folio": dentro.folio, "taller": taller_nombre,
        })

    # Tampoco dos solicitudes vivas para la misma unidad: el vehiculo es uno.
    viva = (db.query(m.SolicitudIngreso)
            .filter(m.SolicitudIngreso.unidad_id == unidad.id,
                    m.SolicitudIngreso.estado.in_(("pendiente", "en_cola")))
            .first())
    if viva:
        raise HTTPException(409, {
            "mensaje": f"Ya tienes una solicitud {viva.estado} para la unidad "
                       f"{unidad.num_economico}. Espera la respuesta del taller.",
            "solicitud_id": viva.id, "estado": viva.estado,
        })
...
def prestar_unidad(datos: PrestamoIn, usuario=Depends(solo_chofer),
    ...
    # La devolucion no puede quedar en el pasado. Se compara contra el dia
    # operativo de Tijuana, no contra UTC: despues de las 17:00 locales el
    # servidor ya esta en el dia siguiente y rechazaria un "hoy" valido.
    hoy = dia_operativo(ahora_utc())
    if datos.fecha_fin_prevista < hoy:
        raise HTTPException(
            400,
            f"La fecha de retorno no puede ser anterior a hoy ({hoy:%d/%m/%Y}). "
            "Elige hoy o una fecha posterior.")
...
def aceptar_prestamo(prestamo_id: int, usuario=Depends(solo_chofer),
    ...
    p.estado = "activo"
    p.fecha_aceptacion = ahora_utc()
    p.fecha_inicio = ahora_utc()
    svc.sincronizar_poseedor(db, p.unidad)   # <-- transferencia de responsabilidad
...
def devolver_unidad(prestamo_id: int, usuario=Depends(solo_chofer),
    ...
    svc.sincronizar_poseedor(db, p.unidad)   # vuelve al titular
...
# ---------------------------------------------------------------- CU-CHO-17 -- #
# v2.0: ya no hay penalizacion ni inconformidad. El sistema AVISA y la
# conversacion con el gerente es el canal de defensa, no un tramite en la app.
@router.get("/avisos", response_model=list[PenalizacionOut])
def mis_avisos(usuario=Depends(solo_chofer), db: Session = Depends(get_db)):
    """CU-CHO-17: el chofer ve sus avisos para enterarse antes de que le hablen."""
    avs = (db.query(m.AvisoIncumplimiento)
           .filter(m.AvisoIncumplimiento.chofer_id == usuario.id)
           .order_by(m.AvisoIncumplimiento.fecha_generacion.desc()).all())
    return [{"id": a.id,
             "unidad": a.unidad.num_economico if a.unidad else "-",
             "chofer": usuario.nombre_completo,
             "motivo": (f"Mantenimiento vencido {a.dias_atraso} dias. "
                        f"Responsable por {a.fundamento_poseedor or 'titularidad'}."),
             "fecha_generacion": a.fecha_generacion,
             "dias_atraso": a.dias_atraso,
             "estado": a.estado} for a in avs]


# Ruta vieja: se conserva para no romper la pantalla actual mientras se migra.
@router.get("/penalizaciones", response_model=list[PenalizacionOut], deprecated=True)
def mis_penalizaciones(usuario=Depends(solo_chofer), db: Session = Depends(get_db)):
    return mis_avisos(usuario, db)
